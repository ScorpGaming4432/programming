...
// Comments can also be written using the `/* ... */` syntax, which allows for multi-line comments. In this project, we are using single-line comments for optional explanations, and multi-line comments for more detailed explanations.
// This is a common practice in C programming to enhance code readability and maintainability. (a.k.a. "self-documenting code")
